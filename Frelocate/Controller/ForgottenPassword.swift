...
//  Frelocate
//
//  Created by Rob Prior on 14/12/2017.
    ...
    @IBAction func submitButtonPressed(_ sender: Any) {
        if emailField.text == nil {
            ...
            FIRAuth.auth()?.sendPasswordReset(withEmail: emailField.text!, completion: { (error) in
                if error != nil {
                    let alert = UIAlertController(title: "Oops, it doesnt happen all the time, I promise", message: "Please try again later", preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: "OK", style: .cancel, handler: nil))
